...
```{r sleepxy,echo=FALSE,fig.height=4.2,fig.align='center'}
...
```{r Sl1,echo=FALSE,fig.height=4.5,fig.align='center'}
...
```{r sleepZ,echo=FALSE,fig.height=3,fig.align='center'}
...
# Fitting the model
```{r sm1} 
summary(fm1 <- lmer(Reaction ~ 1 +  Days + (1+Days|Subject), sleepstudy), corr=FALSE)
``` 
...
```{r fm2,echo=FALSE}
summary(fm2 <- lmer(Reaction ~ 1 + Days + (1|Subject) + (0+Days|Subject), sleepstudy), corr=FALSE)
``` 
...
```{r rr2plot,echo=FALSE,fig.height=5.4,fig.align='center'}
...
```{r shrinkage,echo=FALSE,fig.align='center',fig.height=5.2}
...
```{r shrinkfit,echo=FALSE,fig.height=4,fig.align='center'}
print(xyplot(Reaction ~ Days | Subject, sleepstudy, aspect = "xy",
             layout = c(9,2), type = c("g", "p", "r"),
             coef.list = df[,3:4],
...
```{r caterpillar,echo=FALSE,fig.align='center',fig.height=4.5}
...
- Data from a multi-location trial of several treatments are
    ...
    Wolfinger (1996) **SAS System for Mixed Models** and are
...
```{r Multiplot1,echo=FALSE,fig.height=4.2,fig.align='center'}
...
```{r Multiplot2,echo=FALSE,fig.align='center',fig.height=4.6}
...
```{r fm3}
summary(fm3 <- lmer(Adj ~ 1 + Trt + (1|Grp), Multilocation), corr=FALSE)
```   
...
```{r fm6}
anova(fm6 <- lmer(Adj ~ 1 + Location*Trt + (1|Grp), Multilocation))
anova(fm5, fm6)
```   
...
```{r fm7}
summary(fm7 <- lmer(Adj ~ Trt + (1|Location) + (1|Grp), Multilocation), corr = FALSE)
```   
...
```{r fm8}
fm8 <- lmer(Adj ~ 1 + Trt + (1|Location), Multilocation)
anova(fm8, fm7)
```   
...
```{r fm9}
summary(fm9 <- lmer(Adj ~ Trt + (1|Trt:Location) + (1|Location), Multilocation, REML=FALSE), corr=FALSE)
```   
...
```{r fm10}
summary(fm10 <- update(fm9, . ~ . + (1|Grp), control=lmerControl(calc.derivs = FALSE)), corr=FALSE)
```   
...
```{r fm11a,eval=FALSE}
fm11 <- lmer(Adj ~ 1 + Trt + (1 + Trt|Location) + (1|Grp), Multilocation, REML=FALSE)
``` 
but interpretation is easier when fit as
```{r fm11b,eval=FALSE}
fm11 <- lmer(Adj ~ 1 + Trt + (0+Trt|Location) + (1|Grp), Multilocation, REML=FALSE)
``` 
...
fm11 <- lmer(Adj ~ 1 + Trt + (0+Trt|Location) + (1|Grp), Multilocation, REML=FALSE, verbose=TRUE)
```   
```{r fm11}
getME(fm11,"theta")
```   
...
```{r EarlyData,echo=FALSE,fig.align='center',fig.height=5.2}
...
# Peculiarities of the early childhood cognitive development
- The cognitive score is apparently adjusted for age but
```{r EarlyDensity,echo=FALSE,fig.align='center',fig.height=5.2}
print(densityplot(~ cog | factor(age), Early,  group=trt,
                  layout = c(1,3),
                  plot.points=FALSE,
                  strip=FALSE,
                  strip.left=TRUE,
                  aspect = 0.15,
                  xlab = "Cognitive development score",
...
fm12 <- lmer(cog ~ 1 + tos + trt:tos + (1 + tos|id), Early, control=lmerControl(calc.derivs = FALSE))
```   
...
# Random effects from the Early model fit

```{r fm12re,echo=FALSE,fig.height=5.4}
print(plot(ranef(fm12))[1])
```
